...
if CUR_KPT > 0:
    f.write(u"<p>Oblast propátrají přibližně za " + str(int(math.ceil((P2_P3_P5_KPT + P1_P4_P8_KPT) / float(CUR_KPT)))) + u" h.\n")
if KPT_PT > 0:
    P1_P4_P8_PT = float(SUM_P1) / (float(7) / float(20)) + float(SUM_P4) / (float(7) / float(20)) + float(SUM_P8) / (float(5) / float(20))
    f.write(u"<p>Součástí je prostor, kde je možno KPT nahradit PT. Jedná se o " + str(int(math.ceil(KPT_PT))) + u" ha.\n")
if (SUM_P2 + SUM_P1) > 0:
    f.write(u"<p>Součástí je prostor vhodný pro APT (vrtulník, dron) o rozloze " + str(int(math.ceil(SUM_P2 + SUM_P1))) + u" ha.\n")
